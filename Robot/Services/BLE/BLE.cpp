...
void BLE::setup() {
  ...
  HAL_Delay(5000);
  HAL_UART_Transmit(&BLE_BUS, (uint8_t *)start, strlen(start), timeout);

  // (I) Set name
  HAL_Delay(1000);

  HAL_UART_Transmit(&BLE_BUS, (uint8_t *)setName, strlen(setName), timeout);

  // (II) Set pin
  HAL_Delay(1000);
  HAL_UART_Transmit(&BLE_BUS, (uint8_t *)setPin, strlen(setPin), timeout);

  // (III) Set baud rate
  HAL_Delay(1000);
  HAL_UART_Transmit(&BLE_BUS, (uint8_t *)setBaudRate, strlen(setBaudRate),
                    timeout);
