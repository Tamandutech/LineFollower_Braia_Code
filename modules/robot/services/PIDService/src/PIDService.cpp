...
void PIDService::Run()
{
    for (;;)
    {
        ...
        if (estado != CAR_STOPPED)
        {
            Kp = (PIDClassic->Kp(RealTracklen) != nullptr) ? PIDClassic->Kp(RealTracklen)->getData() : 0;
            Kd = ((PIDClassic->Kd(RealTracklen) != nullptr) ? PIDClassic->Kd(RealTracklen)->getData() : 0) / TaskDelaySeconds;
        ...
        PIDClassic->erro->setData(erro);
        soma_erro += (erro / 1000.0) * (erro / 1000.0);
        PIDClassic->erroquad->setData(soma_erro);
        ...
        I += Ki * erro;
        Pid = P + I + D;
        PIDClassic->output->setData(Pid);
        PIDClassic->P_output->setData(P);
        PIDClassic->D_output->setData(D);
