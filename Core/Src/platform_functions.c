...
#include "adc.h"
#undef ADC_EXPOSE_PRIVATE_BUFFERS
...
//         adc1_update_time = time - last_adc1_time;  // Calcula o tempo de
//         atualização last_adc1_time = time;                     // Atualiza o
//         tempo da última conversão
//     } else if (hadc->Instance == ADC2) {
//         adc2_update_time = time - last_adc2_time;  // Calcula o tempo de
//         atualização last_adc2_time = time;                     // Atualiza o
//         tempo da última conversão
//     }
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim) {
  if (htim->Instance == TIM3) { // Encoder esquerdo
    // Verifica se é overflow (contando para cima) ou underflow (contando para
    // baixo)
    ...
    encoder_u32bit_left = ((uint32_t)encoder_overflow_left << 16) +
    ...
    encoder_u32bit_right = ((uint32_t)encoder_overflow_right << 16) +
                           (uint16_t)__HAL_TIM_GET_COUNTER(htim);
  }
}

// Definições comuns a serem usadas no main loop e demais arquivos
// interplataforma
...
void mcu_start(void) {
  ...
  __HAL_TIM_ENABLE_IT(
      &htim3, TIM_IT_UPDATE); // Habilita interrupção de overflow/underflow
  __HAL_TIM_ENABLE_IT(
      &htim4, TIM_IT_UPDATE); // Habilita interrupção de overflow/underflow
...
void toggle_pin(pinhandler_t pin) { HAL_GPIO_TogglePin(pin.port, pin.pin); }
...
void set_right_encoder_position(uint32_t rightEncoderValue) {
  ...
  __HAL_TIM_SET_COUNTER(
      &htim4, (uint16_t)(rightEncoderValue & 0xFFFF)); // Motor Direito
}
...
void set_left_encoder_position(uint32_t leftEncoderValue) {
  ...
  __HAL_TIM_SET_COUNTER(
      &htim3, (uint16_t)(leftEncoderValue & 0xFFFF)); // Motor Esquerdo
}

uint32_t get_left_encoder_position() {
  encoder_u32bit_left =
      ((uint32_t)encoder_overflow_left << 16) +
      (uint16_t)__HAL_TIM_GET_COUNTER(&htim3); // Motor Esquerdo
  return encoder_u32bit_left;
}

uint32_t get_right_encoder_position() {
  encoder_u32bit_right =
      ((uint32_t)encoder_overflow_right << 16) +
      (uint16_t)__HAL_TIM_GET_COUNTER(&htim4); // Motor Direito
...
float get_battery_voltage(uint32_t *adc_buffer) {
  ...
  float vref = 1.21f * 4095.0f /
               adc_buffer[16]; // Vref = 1.2V, 4095 é o valor máximo do ADC
  float vbat = (adc_buffer[17] * vref / 4095.0f) *
               6.015f; // 5.6875 é a constante do divisor de tensão
  return vbat;         // Retorna a tensão da bateria
}

/** Please note that is MANDATORY: return 0 -> no Error.**/
int32_t write_imu(void *handle, uint8_t reg, const uint8_t *bufp,
                  uint16_t len) {
